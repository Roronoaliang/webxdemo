...
//						System.out.println("每秒发送速度：" + netSendSpeed + " K/s");
						...
						if (netReceiveHasAlarmTime >= netReceiveAlarmLastTime) {
							SystemMonitorBean SystemMonitorBean = new SystemMonitorBean(MyDateUtil.getNowStringDate(),"警告！接收网速在连续 " + netReceiveAlarmLastTime+ "s 内都超过阀值 '" + netReceiveAlarmSpeed+ "K/s' 。");
							queue.add(SystemMonitorBean);
